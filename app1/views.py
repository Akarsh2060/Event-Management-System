...
from app1.utils import send_otp_sms, send_otp_email
import random
import pyotp
...
def resend_otp(request):
    ...
    if secret and phone:
        ...
        print(f"[{timezone.now()}] Resent OTP to {email}: {new_otp}")
...
def send_login_otp(request):
    ...
    try:
        if "@" in identifier:
            profile_obj = tbl_register.objects.get(email=identifier)
        else:
            profile_obj = tbl_register.objects.get(phone=identifier)
